...
review_details <- data_study_general %>%
  select(study_id, date_published, country, review_version) %>%
  write_rds(here::here('data_clean', 'review_details.rds'))

table_1 <- table_1 %>%
  clean_data() %>%
  mutate(., lower_range = sub('\\_.*', '', .$range),
         upper_range = sub('.*_', '',.$range,),
         study_id = 1:length(lead_author)) %>%
  select(1:9, 22:23, 11:21, 24) %>%
  filter(!is.na(lead_author)) %>%
  left_join(., review_details, by = 'study_id') %>%
